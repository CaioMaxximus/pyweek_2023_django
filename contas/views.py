...
def definir_contas(request):
    if request.method == "GET":
        ...
        conta = ContaPagar( titulo=titulo, categoria_id=categoria, 
...
def ver_contas(request):
    ...
    return render(request, 'ver_contas.html', {'contas_vencidas': contas_vencidas, 
                                               ...
                                               'contas_restantes': restantes})
    
    
def pagar_conta(request, id_conta):
    conta_a_pagar = ContaPagar.objects.get(id = id_conta)
    if(conta_a_pagar):
        conta_paga = ContaPaga(conta_id = conta_a_pagar.id,
                               data_pagamento = datetime.now())
        conta_paga.save()
        messages.add_message(request, constants.SUCCESS, 'Conta paga com sucesso')
    else:
        messages.add_message(request, constants.ERROR, 'Erro no pagamento')
